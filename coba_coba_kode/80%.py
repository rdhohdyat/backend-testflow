...
# Process loop body
            ...
            for i, stmt in enumerate(node.body):
                ...
                loop_body_end_nodes.extend(exit_nodes)
                
                # Perbaharui last body node (ambil salah satu ujungnya saja agar chaining berlanjut)
                last_body_node_id = exit_nodes[-1] if exit_nodes else last_body_node_id

            # Create back edges only from actual end points
            for end_node in loop_body_end_nodes:
                if end_node and end_node != loop_id:
                    create_edge(end_node, loop_id, "loop back", is_loop=True)
