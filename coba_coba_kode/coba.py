...
def extract_cfg(tree):
    ...
    def visit(node, parent_id=None, depth=0, branch_index=0, is_else=False):
        if isinstance(node, ast.FunctionDef):
            ...
            # Create edge to true branch
            if node.body:
                first_true = visit(node.body[0], None, depth + 1, branch_index + 1)
                if isinstance(first_true, dict) and "entry_point" in first_true:
                    create_edge(if_id, first_true["entry_point"], "true", False, "true")
                else:
                    create_edge(if_id, first_true, "true", False, "true")
            ...
            if node.orelse:
                ...
                # Create edge to false branch
                first_false = visit(node.orelse[0], None, depth + 1, branch_index + 1, True)
                if isinstance(first_false, dict) and "entry_point" in first_false:
                    create_edge(if_id, first_false["entry_point"], "false", False, "false")
                else:
                    create_edge(if_id, first_false, "false", False, "false")
            ...
            # Create edge to loop body
            if node.body:
                first_body = visit(node.body[0], None, depth + 1, branch_index + 1)
                if isinstance(first_body, dict) and "entry_point" in first_body:
                    create_edge(loop_id, first_body["entry_point"], "true", False, "true")
                else:
                    create_edge(loop_id, first_body, "true", False, "true")
